...
@Override
  ...
  public Integer saveObj(User user) throws SQLException {
    ...
    if(user.getUser_id() != null && user.getUser_id() > 0){
        sql = "update users_tbl set user_firtsname = ?, user_lastname = ?, user_email = ?, user_password = aes_encrypt(?, '$2a$12$Qh2pwy7tAlQEnzRLhFWpYuS12kIL0UBsK6uWEt34/hdciFze9tUyu') where user_id = ?";
    } else{
        sql = "insert into users_tbl (user_firtsname, user_lastname, user_email, user_password)"+ 
        "values ( upper(?), upper(?), lower(?)," +
        "aes_encrypt(?,'$2a$12$Qh2pwy7tAlQEnzRLhFWpYuS12kIL0UBsK6uWEt34/hdciFze9tUyu'));";
    }
